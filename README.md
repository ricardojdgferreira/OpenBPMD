...
## Updates on openbpmd_fixed.py
1) equilibrate function: restraints now use CustomExternalForce('k*periodicdistance(x, y, z, x0, y0, z0)^2') while excluding everything that is not protein or ligand (e.g., water, ions, lipids) (currently hard-coded for POPC and DMPC - two of the most used ones). This allows that membrane proteins and/or CHARMM forcefield to be used adequately
2) New fingerprint interaction function added (can be used instead of ContactScore with keyword "--score fingerprint") (based on TTMD interaction fingerprinting scoring)
