...
descriptn = \
    ...
    Version 1.0.3
      >> refactored how cent_eq_pdb is outputted when using .gro (to adress proper chainIDs)
      >> refactoring of how restraints are applied (now done properly)
      >> no selection needed when using .gro as input: it is assumed that:
         - protein: all except lig_chain
         - ligand: lig_chain
         (this is due to OpenMM automatically assigning chainID while saving PDBs)
    """
...
def main(args):
    ...
    if os.path.isfile(eq_pdb) and not os.path.isfile(cent_eq_pdb):
        if args.structure.endswith('.gro'):
            u_pdb = mda.Universe(eq_pdb, args.structure)
            u_gro = mda.Universe(args.structure)
            for res_pdb, res_gro in zip(u_pdb.residues, u_gro.residues):
                res_pdb.resid = res_gro.resid
            u_pdb.trajectory.add_transformations(wrap(u_pdb.atoms, compound='residues'))
            for seg in u_pdb.segments:
                if seg.residues.resnames[0] in ["WAT", "HOH", "SOL"]:
                    seg.segid = "W"
                elif seg.residues.resnames[0] in ["NA", "CL", "CA"]:
                     seg.segid = "I"
            chainIDs = u_pdb.atoms.chainIDs
            segids = u_pdb.atoms.segids
            for atom, segid in zip(u_pdb.atoms, segids):
                atom.chainID = segid
            for seg in u_pdb.segments:
                seg.segid = ""
            u_pdb.atoms.write(cent_eq_pdb)
        else:
    ...
    for idx in range(0, args.nreps):
        ...
        print("Running production ", str(idx))        
        produce(args.output, idx, eq_pdb, parm, args.parameters, args.structure, args.lig_chain, args.hill_height)
                
        trj_name = os.path.join(rep_dir,'trj.dcd')               
        PoseScoreArr = get_pose_score(cent_eq_pdb, trj_name, args.lig_chain)
        ContactScoreArr = get_contact_score(cent_eq_pdb, trj_name, args.lig_chain)
...
def get_contact_score(structure_file, trajectory_file, lig_chain):
    ...
    # then load the trajectory and get selections
    sel_donor = f"protein and chainID {lig_chain} and not name *H*"
    sel_acceptor = f"(protein and not chainID {lig_chain} and not name H*) and (around 5 chainID {lig_chain})"

    a_donors = u.select_atoms(sel_donor)
    a_acceptors = u.select_atoms(sel_acceptor)

    # calculate contacts
    cont_analysis = contacts.Contacts(u, select=(sel_donor, sel_acceptor), refgroup=(a_donors, a_acceptors), radius=3.5).run()
...
def get_pose_score(structure_file, trajectory_file, lig_chain):
    ...
    r = rms.RMSD(u, select=f"protein and chainID {lig_chain} and not name H*", ref_frame=0).run()
...
def minimize(parm, input_positions, out_dir, min_file_name):
    ...
    """
    system = parm.createSystem(nonbondedMethod=PME, nonbondedCutoff=1*nanometers, constraints=HBonds,)
...
def produce(out_dir, idx, eq_pdb, parm, parm_file, coords_file, lig_chain, set_hill_height):
    ...
    prot_com = universe.select_atoms(f"protein and not chainID {lig_chain}").center_of_mass()
    ...
    lig = universe.select_atoms(f"protein and chainID {lig_chain} and not name *H*")
    lig_ha_idx = lig.indices.tolist()

    # Set up the system to run metadynamics
    system = parm.createSystem(nonbondedMethod=PME, nonbondedCutoff=1*nanometers, constraints=HBonds, hydrogenMass=4*amu)
    ...
    meta = Metadynamics(system, [rmsd_cv], 300.0*kelvin, 4.0, hill_height, 250, biasDir=write_dir, saveFrequency=250000)
    ...
    simulation.reporters.append(StateDataReporter(sys.stdout, 25000,
...
if __name__ == "__main__":
    ...
    parser.add_argument("-lig_chain", type=str, default='B',
                        help='the chainID of the ligand (default: %(default)s)')
